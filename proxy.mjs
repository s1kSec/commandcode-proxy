...
$('loginForm').addEventListener('submit',async event=>{event.preventDefault();const button=event.submitter;button.disabled=true;try{await api('/api/admin/login',{method:'POST',body:JSON.stringify({password:$('password').value})});render(await api('/api/admin/state'));$('password').value='';message('登录成功')}catch(error){showLogin();message(error.message,true)}finally{button.disabled=false}});
